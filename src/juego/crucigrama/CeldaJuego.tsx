...
/**
 * Celda de crucigrama JUGABLE (C-10, D4/6.1).
 *
 * Estados visuales:
 * - `negra`: estática, sin interacción (bg-ink/60).
...
export function CeldaJuego({
  ...
  const base = `relative flex h-9 w-9 items-center justify-center rounded-md font-display text-lg font-semibold sm:h-11 sm:w-11 ${contorno} ${fondo}`;
  ...
  if (activa) {
    return (
      <div className={base} role="gridcell" onClick={onCeldaClick}>
        {numeroVisible}
        <input
          ref={inputRef}
          value={letra}
